...
void MemoryController::atomic()
{
    ...
    for (iter=m_pagequeue->begin(); iter!=m_pagequeue->end();){
        ...
        if (m_clock >= exectick){
            ...
            if (!(m_dramsim[calcModuleIdx(waddr)]->WillAcceptTransaction(calcModuleAddr(waddr), dpacket.is_write))){
            ...
            m_dramsim[calcModuleIdx(waddr)]->AddTransaction(calcModuleAddr(waddr), dpacket.is_write);
